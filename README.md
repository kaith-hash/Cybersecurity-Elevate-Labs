...
### **Objective**
  ...
  * Identify phishing characteristics in a suspicious email sample[cite: 57].
  * Utilize an email client and a free online header analyzer[cite: 58].
  * Learn to systematically identify phishing tactics and analyze email threats.

### **Tools Used**

  * **Email Client** or saved email file (text) [cite: 58]
  * **Free Online Header Analyzer** (e.g., MXToolbox) [cite: 58]

### **Key Concepts**

| Concept | Description |
| :--- | :--- |
| **Phishing** | Fraudulently acquiring sensitive data by disguising communication as trustworthy[cite: 60]. |
| **Email Spoofing** | Forging the sender's address, often using **lookalike domains** (e.g., `bannk` instead of `bank`)[cite: 60, 64]. |
| **Header Analysis** | Examining the raw headers to check authentication (SPF, DKIM, DMARC) and the true sending IP[cite: 60, 68]. |
| **Social Engineering** | Psychological manipulation using **urgency** or **threats** to induce immediate action[cite: 60, 104]. |
| **Threat Detection** | Identifying indicators like failed authentication, mismatched URLs, and malicious attachments[cite: 60, 114, 116, 118]. |
...
The raw header is pasted into a tool like the MXToolbox Email Header Analyzer[cite: 86].
...
2.  **Check Attachments:**
      ...
      * **Inside (simulated):** `Invoice_Sept2025.exe`[cite: 98].
      * **Finding:** Attachments with **double extensions or executables inside compressed files (.zip)** are common malware delivery vectors[cite: 99, 118]. **Do not open** attachments[cite: 100].
